...
const useDocumentVisibility = () => {
  ...
  useEffect(() => {
    window.addEventListener(getValidBrowser()[0], () => setIsVisible(!getValidBrowser()[1]));
    return () => window.removeEventListener('visibilitychange', () => setIsVisible(!getValidBrowser()[1]));
  });
  return isVisible;
};
